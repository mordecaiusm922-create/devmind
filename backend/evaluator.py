...
_COMPILED_RISK_RULES  = [
    (re.compile(p, re.IGNORECASE), floor, tag)
...
def _build_triage(summary: dict, risk_score: int, ev: "Evaluation") -> tuple[str, bool]:
    ...
    if "critical" in severities or risk_score >= P0_THRESHOLD: return "P0", True
    if "high" in severities      or risk_score >= P1_THRESHOLD: return "P1", True
    if risk_score >= P2_THRESHOLD:                              return "P2", False
    return "P3", False
...
def _usefulness_check(summary: dict) -> UsefulnessCheck:
    ...
    missing: list[str] = []
    if not has_filenames:        missing.append("file names")
    if not has_functions:        missing.append("function or identifier names")
    if not has_specific_changes: missing.append("concrete change verbs")
