...
def _pipeline_sync(repo: str, pr_number: int, trace_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if _can_approve and not _has_real_findings and not _ast2 and not _crit2 and not _has_verification_required and 'verification_violations' not in response.get('why_chain', []):
            response["decision"] = {
                "action": "ALLOW",
                "reason": "Policy engine auto-approved: trivial surface with no security signals.",
                "confidence": 0.9,
