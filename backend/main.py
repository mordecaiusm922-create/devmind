    ...
    if event == "pull_request" and data.get("action") in ["opened", "synchronize"]:
        ...
        async def analyze_and_comment():
            try:
                ...
                re = risk
                risk_emoji = {"low": "🟢", "medium": "🟡", "high": "🔴", "critical": "🟣"}.get(risk_obj.get("level", "low"), "⚪")
...
"
                    for v in vulns:
                        vuln_text += f"> **{v.get('severity','').upper()}** {v.get('location','')}
> {v.get('description','')}
> **Fix:** {v.get('fix','')}

"
                factors = "
".join(f"- {f}" for f in (re.top_factors or []))
                body = f"""## 🤖 DevMind Analysis

### {risk_emoji} Risk: {risk_obj.get('level', 'unknown').upper()}
{risk_obj.get('reason', '')}
...
{vuln_text}
### 📊 Risk Score: {re.risk_score}/100 — {re.risk_label}
{factors}

---
*Generated by [DevMind](https://devmind-gamma.vercel.app) — AI-powered PR risk engine*"""
