...
_SECRET_ASSIGN_RE = re.compile(r"(?im)^\+?\s*(SECRET_KEY|PASSWORD|TOKEN|API_KEY|PRIVATE_KEY)\s*=\s*['\"][^'\"]{8,}['\"]")
...
_REMOVED_AUTH_RE = re.compile(r"(?im)^-\s*(if|raise|return).*?(is_admin|permission|authorize|can_|policy|rbac)")
...
_RULES: tuple[dict[str, Any], ...] = (
    ...
    {"id": "data_leak_or_dos", "surface": "data", "severity": "high", "property": "data_safety_policy", "pattern": r"console\.log\(.*password|logger\..*(ssn|password|token)|select \*|return user\.__dict__|pagination\s*=\s*false|limit\s*=\s*none|backup.*public|backup.*encryption.*false|n_plus_one|for .* in .*:.*query"},
    {"id": "runtime_regression", "surface": "runtime", "severity": "high", "property": "runtime_safety_policy", "pattern": r"timeout\s*=\s*0|while true|retry.*while|backoff\s*=\s*false|cache\.invalidate.*removed|traceback\.format_exc\(\)|debug\s*=\s*true|except exception:\s*pass|rollback\s*=\s*none"},
    ...
    {"id": "insider_or_process_risk", "surface": "human", "severity": "high", "property": "human_review_policy", "pattern": r"branch_protection\s*=\s*false|direct push to main|self[-_ ]approved|audit.*logs.*removed|shared credentials|service_account.*admin|secret access.*false"},
)
_COMPILED_RULES = [{**rule, "regex": re.compile(rule["pattern"], re.IGNORECASE | re.MULTILINE)} for rule in _RULES]
...
def run_safety_flow(req: SafetyFlowRequest) -> dict[str, Any]:
    ...
    if req.repo:
        try:
            ...
            prior_data = get_prior_for_prompt(req.repo, req.prompt) or {}
        except Exception:
            prior_data = {}
    ...
    verifications = {c["id"]: _verify_candidate(c, properties) for c in candidates}
    runtime_evidence = {c["id"]: _runtime_evidence(c, properties, representation) for c in candidates}
    ...
    ranking = _rank_candidates(evaluation.get("scores", {}), verifications, runtime_evidence, prior_data, representation, candidates)
    ...
    evaluation["requires_verification"] = bool(evaluation.get("requires_verification") or (selected and (selected.get("violations") or selected.get("critical_violations"))))
    evaluation["requires_repair"] = bool(evaluation.get("requires_repair") or (selected and selected.get("critical_violations")))
    ...
    recorded = _record_flow_result(req, selected, decision, representation, operational_metrics, risk)

    return {
        "flow": ["observe", "retrieve", "generate", "score", "verify", "repair", "risk_adjust", "rank", "decide", "record"],
        "policy": {
...
def _infer_intent(prompt: str, context: dict[str, Any]) -> dict[str, Any]:
    text = f"{prompt} {json.dumps(context, default=str)}".lower()
...
def _infer_properties(prompt: str, context: dict[str, Any], explicit: list[str], representation: dict[str, Any]) -> list[str]:
    ...
    if "ci_cd" in surface:
        props.append("no_untrusted_secret_access")
    if "infra" in surface:
        props.append("explicit_rollback_path")
...
def _generate_candidates(prompt: str, context: dict[str, Any], n: int) -> list[dict[str, Any]]:
    ...
    if any(k in text for k in ("sql", "query", "injection")):
        templates = [
            ("parameterized-sql", '+cursor.execute("SELECT * FROM users WHERE email = %s", [email])', "Use bound parameters so user input never changes SQL structure."),
            ("orm-filter", "+user = User.objects.filter(email=email).first()", "Use ORM query construction to avoid raw SQL interpolation."),
            ("validate-and-parameterize", '+validate_email(email)\n+cursor.execute("SELECT * FROM users WHERE email = %s", [email])', "Validate input shape and use parameterized SQL."),
        ]
    elif any(k in text for k in ("auth", "authorization", "permission", "rbac", "is_admin")):
        templates = [
            ("policy-check", '+if not policy.can_perform(user, resource):\n+    raise PermissionError("not authorized")\n+return perform_action(resource)', "Fail closed through a centralized authorization policy."),
            ("explicit-admin-guard", '+if not user.is_admin:\n+    raise PermissionError("admin required")\n+return perform_action(resource)', "Add an explicit permission guard before the sensitive action."),
            ("decorator-guard", '+@requires_permission("resource:write")\n+def handler(request):\n+    return perform_action(resource)', "Protect the entrypoint with a permission decorator."),
        ]
    elif any(k in text for k in ("secret", "token", "api_key", "password")):
        templates = [
            ("env-fail-fast", '+import os\n+SECRET_KEY = os.environ.get("SECRET_KEY")\n+if not SECRET_KEY:\n+    raise RuntimeError("SECRET_KEY environment variable is not set")', "Read secret from the environment and fail closed if it is missing."),
            ("env-only", '+import os\n+SECRET_KEY = os.environ.get("SECRET_KEY")', "Read secret from the environment with a minimal change."),
            ("config-loader", '+SECRET_KEY = config.require_secret("SECRET_KEY")', "Use a config helper that requires secret material at runtime."),
        ]
    elif any(k in text for k in ("terraform", "kubernetes", "k8s", "iam", "s3", "security group", "rds", "docker", "infra", "cloud", "public", "privileged")):
        templates = [
            ("least-privilege-infra", '+# restrict public access and wildcard permissions\n+policy = least_privilege(policy)\n+rollback = "terraform plan && revert previous module version"', "Constrain cloud permissions, remove public exposure, and require rollback."),
            ("private-networking", '+cidr_blocks = [var.private_cidr]\n+publicly_accessible = false\n+rollback = "restore previous security group"', "Move exposed infrastructure behind private networking."),
            ("pinned-runtime", '+image = "service@sha256:verified-digest"\n+allowPrivilegeEscalation = false\n+rollback = "rollout undo deployment/service"', "Pin runtime artifacts and disable privileged execution."),
        ]
    elif any(k in text for k in ("pull_request_target", "github actions", "workflow", "ci/cd", "supply chain", "curl | bash", "artifact", "secrets.")):
        templates = [
            ("ci-readonly-pinned", '+permissions:\n+  contents: read\n+# pin actions by sha and do not expose secrets to untrusted jobs', "Use read-only workflow permissions, pin actions, and avoid secrets in untrusted contexts."),
            ("verified-build", '+run: npm ci && npm test\n+# verify artifact signature before deploy', "Restore tests and artifact verification in the deploy pipeline."),
            ("trusted-dependency-source", '+# install dependencies from the official registry with lockfile verification', "Avoid unverified forks and install scripts in the build path."),
        ]
    elif any(k in text for k in ("dependency", "cve", "npm", "package", "requirements", "license", "typosquat", "postinstall")):
        templates = [
            ("dependency-upgrade-pin", '+# upgrade vulnerable dependency, pin exact version, and verify lockfile hash', "Upgrade or remove vulnerable packages and pin the safe version."),
            ("dependency-audit-gate", '+# block build on critical CVE, malicious package, license violation, or install-time network access', "Add a dependency policy gate for CVEs, licensing, and install behavior."),
            ("remove-risky-package", '+# remove unofficial fork and replace with maintained official package', "Replace risky dependencies with maintained and trusted packages."),
        ]
    elif any(k in text for k in ("migration", "drop column", "pii", "pagination", "backup", "index", "n+1", "soft delete", "data leak")):
        templates = [
            ("data-safety-guard", '+# require backup, pagination, field redaction, and rollback before data migration', "Guard data changes with backup, redaction, pagination, and rollback."),
            ("safe-migration", '+# expand-contract migration: add new column, backfill, verify, then remove in a later deploy', "Convert destructive migration into a staged migration."),
            ("redact-sensitive-output", '+return redact_sensitive_fields(user, fields=["password", "token", "ssn"])', "Prevent sensitive fields and PII from leaving the service boundary."),
        ]
        ...
        templates = [
            ("runtime-guardrails", '+timeout = bounded_timeout(default=30)\n+retry = exponential_backoff(max_attempts=3)\n+rollback = "disable feature flag"', "Restore bounded timeout, backoff, and rollback controls."),
            ("safe-payment-lock", '+with payment_lock(account_id):\n+    charge_once(request)', "Protect financial operations with explicit locking and idempotency."),
            ("safe-error-handling", '+logger.error("request failed", extra=redact_sensitive_context(ctx))\n+return generic_error_response()', "Avoid stack trace and credential exposure in production errors."),
        ]
    elif any(k in text for k in ("llm", "ai", "prompt injection", "embedding", "fine-tune", "model", "langflow", "sandbox")):
        templates = [
            ("ai-sandbox-policy", '+# run LLM tool calls in sandbox, require auth, rate limit, and sanitize HTML output', "Constrain AI execution with sandboxing, auth, rate limits, and output sanitization."),
            ("ai-data-boundary", '+# block PII/secrets from embeddings and fine-tuning datasets; enforce access control', "Protect sensitive data from model training and vector storage."),
            ("secure-token-generation", '+token = secrets.token_urlsafe(32)', "Use cryptographic randomness for security tokens."),
        ]
    elif any(k in text for k in ("branch protection", "main", "self-approved", "audit", "insider", "service account", "employee")):
        templates = [
            ("human-review-policy", '+# require branch protection, independent approval, audit logs, and justified secret access', "Force human governance on insider-risk changes."),
            ("least-privilege-service-account", '+# replace admin service account with scoped role and time-bound access', "Remove shared or admin credentials from development workflows."),
            ("audit-preserving-change", '+# preserve audit logging and require security owner approval', "Prevent removal of audit controls without independent review."),
        ]
    else:
        templates = [
...
def _verify_candidate(candidate: dict[str, Any], properties: list[str]) -> dict[str, Any]:
    ...
    sql_semantics = verify_sql_semantics(diff, require_validate_email="validate_email_present" in properties)
    ...
    for prop in properties:
        if prop == "no_hardcoded_secret":
            ...
            if (_UNSAFE_SQL_RE.search(diff) and not _PARAM_SQL_RE.search(diff)) or bool(sql_semantics.get("critical_violations")):
                violations.append(prop)
                critical.append(prop)
            else:
                evidence.append(prop)
            ...
            has_rollback = any(marker in lower for marker in ("rollback", "rollout undo", "revert", "previous_version", "terraform plan"))
            if not has_rollback:
                violations.append(prop)
            else:
                evidence.append(prop)
        elif prop == "least_privilege_infra":
            unsafe = re.search(r"0\.0\.0\.0/0|public-read|publicly_accessible\s*=\s*true|action\s*=\s*['\"]\*|resource\s*=\s*['\"]\*|privileged\s*:\s*true|:latest", lower, re.IGNORECASE)
            ...
            unsafe = re.search(r"drop column|drop table|select \*|console\.log\(.*password|pagination\s*=\s*false|backup.*public", lower)
    ...
    score = max(0.0, 1.0 - 0.12 * len(violations) - 0.18 * len(critical))
    return {
...
def _rank_candidates(
    ...
    for cid, score in scores.items():
        ...
        expected_loss = (1.0 - utility) * (0.45 + blast_score) + 0.10 * violations + 0.25 * critical + 0.08 * uncertainty + runtime_penalty + graph_penalty
        adjusted = utility + prior_adjustment - blast_radius_penalty - graph_penalty - runtime_penalty - 0.14 * violations - 0.25 * critical - 0.08 * uncertainty + verified_bonus
...
def _final_decision(mode: str, selected: dict[str, Any] | None, evaluation: dict[str, Any], representation: dict[str, Any]) -> dict[str, Any]:
    if selected is None:
    ...
    if selected["critical_violations"]:
        return {"action": "reject", "reason": "Best candidate still has critical safety violations.", "candidate": selected["candidate"], "merge_blocker": True}

    if selected["violations"]:
        return {"action": "revise", "reason": "Best candidate has unresolved verification violations.", "candidate": selected["candidate"], "merge_blocker": False}

    if selected.get("runtime_evidence_status") == "failed":
        return {"action": "revise", "reason": "Runtime evidence sandbox failed for the selected candidate.", "candidate": selected["candidate"], "merge_blocker": False}
    ...
    policy = policy_engine.decide(evaluation=evaluation, selected=selected, mode=mode)
    return {
...
def _build_change_representation(req: SafetyFlowRequest, candidates: list[dict[str, Any]], prior_data: dict[str, Any]) -> dict[str, Any]:
    files = _files_from_request_or_candidates(req, candidates)
    critical_findings = _detect_critical_findings(req.prompt, req.context, files) if req.files or files else []
    ...
    return {
        "repo": req.repo,
        "intent": req.intent or _infer_intent(req.prompt, req.context),
        "files": [{"filename": f.get("filename"), "status": f.get("status", "modified"), "additions": int(f.get("additions") or 0), "deletions": int(f.get("deletions") or 0)} for f in files],
...
def _files_from_request_or_candidates(req: SafetyFlowRequest, candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if req.files:
        normalized = []
        for f in req.files:
    ...
    for candidate in candidates:
        ...
        if filename not in by_file:
            by_file[filename] = {"filename": filename, "diff": "", "raw_patch": "", "status": "modified", "additions": 0, "deletions": 0}
    ...
    return list(by_file.values()) or [{"filename": fallback, "diff": "", "raw_patch": "", "status": "modified", "additions": 0, "deletions": 0}]
...
def _detect_critical_findings(prompt: str, context: dict[str, Any], files: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    findings.sort(key=lambda f: _SEVERITY_RISK.get(str(f.get("severity", "low")), 0.0), reverse=True)
    return findings


def _risk_surface(prompt: str, context: dict[str, Any], files: list[dict[str, Any]]) -> list[str]:
    text = " ".join([prompt, json.dumps(context, default=str), " ".join(str(f.get("filename", "")) for f in files), " ".join(str(f.get("diff", "")) for f in files)]).lower()
...
def _repair_candidates(candidates: list[dict[str, Any]], verifications: dict[str, Any], properties: list[str], context: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for candidate in candidates:
        ...
        repaired.append({
            ...
            "metadata": {"generated_by": "safety_flow_repair", "repaired_from": cid, "fixed_properties": sorted(violations | critical)},
        })

    return repaired


def _repair_patch_for_violations(filename: str, violations: set[str], properties: list[str]) -> dict[str, str] | None:
    if {"no_hardcoded_secret", "secret_from_environment", "fail_fast"} & violations:
        body = '+import os\n+SECRET_KEY = os.environ.get("SECRET_KEY")\n+if not SECRET_KEY:\n+    raise RuntimeError("SECRET_KEY environment variable is not set")'
        return {"strategy": "repair-env-fail-fast", "diff": _format_diff(filename, body), "explanation": "Replace hardcoded secret material with an environment-backed value and fail fast when missing."}

    if {"no_raw_sql", "parameterized_sql", "validate_email_present"} & violations:
        body = '+validate_email(email)\n+cursor.execute("SELECT * FROM users WHERE email = %s", [email])'
        return {"strategy": "repair-parameterized-sql", "diff": _format_diff(filename, body), "explanation": "Validate the email input and use bound parameters so external input cannot change the SQL structure."}

    if {"auth_guard_present", "no_auth_guard_removal", "fail_closed"} & violations:
        body = '+if not policy.can_perform(user, resource):\n+    raise PermissionError("not authorized")\n+return perform_action(resource)'
        return {"strategy": "repair-policy-guard", "diff": _format_diff(filename, body), "explanation": "Restore a fail-closed authorization guard before the sensitive operation."}

    if "no_untrusted_secret_access" in violations:
        body = '+permissions:\n+  contents: read\n+# no secrets are exposed to untrusted pull_request_target jobs'
        return {"strategy": "repair-ci-readonly", "diff": _format_diff(filename, body), "explanation": "Constrain CI permissions and avoid exposing secrets to untrusted workflow contexts."}

    if "least_privilege_infra" in violations:
        body = '+# restrict public access and wildcard permissions\n+policy = least_privilege(policy)\n+publicly_accessible = false\n+rollback = "terraform plan && revert previous module version"'
        return {"strategy": "repair-least-privilege-infra", "diff": _format_diff(filename, body), "explanation": "Remove public exposure and wildcard permissions with an explicit rollback path."}

    if "safe_ci_supply_chain" in violations:
        body = '+permissions:\n+  contents: read\n+# pin actions by sha, run tests, and verify artifact signature before deploy'
        return {"strategy": "repair-safe-ci-supply-chain", "diff": _format_diff(filename, body), "explanation": "Harden CI/CD permissions, action pinning, tests, and artifact verification."}
    ...
    if "dependency_policy" in violations:
        ...
        return {"strategy": "repair-dependency-policy", "diff": _format_diff(filename, body), "explanation": "Apply dependency policy controls for CVEs, pinning, licensing, and install behavior."}
    ...
    if "data_safety_policy" in violations:
        ...
        return {"strategy": "repair-data-safety-policy", "diff": _format_diff(filename, body), "explanation": "Guard data changes with backup, staged migration, redaction, and rollback."}

    if "runtime_safety_policy" in violations:
        body = '+timeout = bounded_timeout(default=30)\n+retry = exponential_backoff(max_attempts=3)\n+rollback = "disable feature flag"\n+logger.error("request failed", extra=redact_sensitive_context(ctx))'
        return {"strategy": "repair-runtime-safety-policy", "diff": _format_diff(filename, body), "explanation": "Restore runtime guardrails for timeout, backoff, rollback, and safe error handling."}
    ...
    if "ai_safety_policy" in violations:
        ...
        return {"strategy": "repair-ai-safety-policy", "diff": _format_diff(filename, body), "explanation": "Constrain AI execution and data access with sandboxing, auth, rate limits, and sanitization."}
    ...
    if "human_review_policy" in violations:
        ...
        return {"strategy": "repair-human-review-policy", "diff": _format_diff(filename, body), "explanation": "Require governance controls for insider-risk or process-risk changes."}
...
def _repair_iteration_deltas(repair_candidates: list[dict[str, Any]], evaluation: dict[str, Any], verifications: dict[str, Any]) -> list[dict[str, Any]]:
...
def _repair_converged(repair_candidates: list[dict[str, Any]], selected: dict[str, Any] | None, verifications: dict[str, Any]) -> bool:
...
def _operational_metrics(ranking: list[dict[str, Any]], verifications: dict[str, Any], runtime_evidence: dict[str, Any], evaluation: dict[str, Any]) -> dict[str, Any]:
    ...
    unsafe_top_k = sum(1 for item in top_k if item.get("violations") or item.get("critical_violations") or not item.get("verified"))
    ...
    mean_uncertainty = sum(float(item.get("uncertainty") or 0.0) for item in top_k) / len(top_k) if top_k else 0.0
...
def _safety_flow_risk(representation: dict[str, Any], selected: dict[str, Any] | None, decision: dict[str, Any]) -> dict[str, Any]:
    findings = representation.get("critical_findings", []) or []
    finding_score = max((int(round(_SEVERITY_RISK.get(str(f.get("severity", "low")), 0.32) * 100)) for f in findings), default=0)
    blast = representation.get("blast_radius") or {}
...
def _runtime_evidence(candidate: dict[str, Any], properties: list[str], representation: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "filename": _candidate_filename(candidate, {"filename": "app.py"}),
        }
...
def _calibrate_risk(repo: str, risk: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        risk["calibration"] = {"method": calibrated["method"], "summary": calibrated["summary"]}
...
def _deployment_policy(mode: str, representation: dict[str, Any], runtime_evidence: dict[str, Any], risk: dict[str, Any], selected: dict[str, Any] | None) -> dict[str, Any]:
    try:
        result = policy_engine.evaluate_deployment(
            representation=representation,
            runtime_evidence=runtime_evidence,
            risk=risk,
            selected=selected,
...
def _apply_deployment_policy(decision: dict[str, Any], risk: dict[str, Any], deployment_policy: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    action = str(deployment_policy.get("action") or "").upper()
    if action == "BLOCK":
        decision = {**decision, "action": "reject", "reason": deployment_policy.get("reason", "Deployment policy blocked this change."), "merge_blocker": True, "deployment_policy_action": "BLOCK"}
        risk = {**risk, "score": max(int(risk.get("score") or 0), 90), "band": "critical", "triage": "P0", "p_exploit": max(float(risk.get("p_exploit") or 0.0), 0.90)}
    elif action == "REVIEW" and decision.get("action") == "approve":
        decision = {**decision, "action": "needs_verification", "reason": deployment_policy.get("reason", "Deployment policy requires review."), "merge_blocker": bool(deployment_policy.get("merge_blocker", False)), "deployment_policy_action": "REVIEW"}
...
def _record_flow_result(req: SafetyFlowRequest, selected: dict[str, Any] | None, decision: dict[str, Any], representation: dict[str, Any], operational_metrics: dict[str, Any], risk_model: dict[str, Any]) -> dict[str, Any] | None:
    if not req.repo:
        return None
